...
process_one_draw <- function(sampled_df){
  ...
  unfixed <- directEST_1030(sampled_df, NULL, admin=2, aggregation=FALSE,
    ...
    mutate(type="unfixed")
  trg <- directEST_1030(sampled_df, NULL, admin=2, aggregation=FALSE,
                       var.fix=TRUE, all.fix=FALSE)$res.admin2 %>%
  ...
  bind_rows(all_unfixed, all_fixed, fixed_illegal_unfixed_legal, unfixed_legal)
}
...
summarise_metrics <- function(df_all, conf_levels=conf_lvls){
  one_level <- function(df, cl){
    ...
    df %>%
      ...
      mutate(
        df_eff = case_when(
          method %in% c("all_unfixed", "fixed_illegal_unfixed_legal", "unfixed_legal") ~ df_unfixed, # use K-H
...
## 3.1 Method Tags & Colors (3 colors only) ----
...
## 3.2 Preprocessing: Retain 3 methods + sort by conf_lvls ----
