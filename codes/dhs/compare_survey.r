...
poly.adm1 <- readRDS("~/Desktop/vairance fix/data/poly.adm1.rds")
poly.adm2 <- readRDS("~/Desktop/vairance fix/data/poly.adm2.rds")
...
# Admin1: direct estimate
cmp_adm1_est <- cmp_adm1_est %>%
  mutate(
    difference = sas_est - survey_est,
    admin1 = factor(admin1, levels = admin1)
  )

p5 <- ggplot(cmp_adm1_est, aes(x = admin1, y = difference)) +
  geom_point(size = 2, color = "blue") +
  geom_hline(
    yintercept = 0,
    linetype = "dashed",
  ...
  labs(
    x = "Admin1",
    y = "Closed form - survey",
    title = "Admin1: direct.est difference"
  ) +
  theme_minimal() +
  theme(
...
# Admin1: direct variance
cmp_adm1_var <- cmp_adm1_var %>%
  mutate(
    difference = sas_var - survey_var,
    admin1 = factor(admin1, levels = admin1)
  )

p6 <- ggplot(cmp_adm1_var, aes(x = admin1, y = difference)) +
  geom_point(size = 2, color = "blue") +
  geom_hline(
    yintercept = 0,
    linetype = "dashed",
  ...
  labs(
    x = "Admin1",
    y = "Closed form - survey",
    title = "Admin1: direct.var difference"
  ) +
  theme_minimal() +
  theme(
    axis.text.x = element_text(angle = 45, hjust = 1)
...
# Admin2: direct estimate
cmp_adm2_est <- cmp_adm2_est %>%
  mutate(
    difference = sas_est - survey_est,
    area_id = row_number()
  )

p7 <- ggplot(cmp_adm2_est, aes(x = area_id, y = difference)) +
  geom_point(size = 2, color = "blue") +
  geom_hline(
    yintercept = 0,
    linetype = "dashed",
  ...
  labs(
    x = "Admin2 area",
    y = "Closed form - survey",
    title = "Admin2: direct.est difference"
  ) +
  theme_minimal()


# Admin2: direct variance
cmp_adm2_var <- cmp_adm2_var %>%
  mutate(
    difference = sas_var - survey_var,
    area_id = row_number()
  )

p8 <- ggplot(cmp_adm2_var, aes(x = area_id, y = difference)) +
  geom_point(size = 2, color = "blue") +
  geom_hline(
    yintercept = 0,
    linetype = "dashed",
  ...
  labs(
    x = "Admin2 area",
    y = "SAS - survey",
    title = "Admin2: direct.var difference"
  ) +
  theme_minimal() +
  coord_cartesian(
    clip = "off"
...
# Combine four panels
sas_vs_survey_difference <- (p5 + p6) / (p7 + p8)

sas_vs_survey_difference
