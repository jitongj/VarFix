...
run_simulations_and_collect_data <- function(admin1_list, admin2_cluster_pop, res_ad1) {
  ...
  purrr::map_dfr(seq_len(n_iterations), function(sim_id) {
    all_results <- sampled_by_admin1 %>%
      purrr::map_dfr(~process_simulation(.x$res$sampled_indiv_list[[sim_id]], .x$true_p_df))
    
    gvf_data <- all_results %>%
      dplyr::filter(type == "fixed_triggered") %>%
      dplyr::mutate(
    ...
    gvf_all <- gvf_data %>%
      dplyr::mutate(
        ...
        type = "GVF",
        method = "GVF",
        df_taylor = df_unfixed_admin1
      )
    
    dplyr::bind_rows(all_results, gvf_all) %>%
...
output_dir <- here::here("figures", "simulation_sensitivity", "admin1Mean_admin1Weight")
dir.create(output_dir, recursive = TRUE, showWarnings = FALSE)

saveRDS(
  all_detailed_results,
  file.path(output_dir, "sensitivityAdmin1MeanWeight_simulation_direct_GVF_detailed_results.rds")
)
saveRDS(
  all_summary_metrics,
  file.path(output_dir, "sensitivityAdmin1MeanWeight_simulation_direct_GVF_performance_summary.rds")
)

# Keep data/sim_f1_s1_p1_r0.042_nationalWeight_GVF.RData (consumed by
# plot_stratified_legal_illegal_GVF_benchmark.R) in sync with this run.
save(
  all_detailed_results,
  all_summary_metrics,
  file = here::here("data", "sim_f1_s1_p1_r0.042_admin1MeanWeight_GVF.RData")
)

ggsave(
  filename = file.path(output_dir, "sensitivityAdmin1MeanWeight_simulation_direct_GVF_coverage.png"),
  plot = p_cov_z,
  width = 14, height = 6, dpi = 300
)
ggsave(
  filename = file.path(output_dir, "sensitivityAdmin1MeanWeight_simulation_direct_GVF_ci_width.png"),
  plot = p_wz,
  width = 14, height = 6, dpi = 300
)
ggsave(
  filename = file.path(output_dir, "sensitivityAdmin1MeanWeight_simulation_direct_GVF_interval_score.png"),
  plot = p_sz,
  width = 14, height = 6, dpi = 300
)
ggsave(
  filename = file.path(
    output_dir,
    "sensitivityAdmin1MeanWeight_simulation_direct_GVF_bias_variance_estimators.png"
  ),
  plot = p_bias_var_est,
  width = 14,
  height = 6,
...
ggsave(
  filename = file.path(
    output_dir,
    "sensitivityAdmin1MeanWeight_simulation_direct_GVF_coverage_95.png"
  ),
  plot = p_cov_z_095,
  width = 14,
  height = 6,
...
ggsave(
  filename = file.path(
    output_dir,
    "sensitivityAdmin1MeanWeight_simulation_direct_GVF_ci_width_95.png"
  ),
  plot = p_wz_095,
  width = 14,
  height = 6,
...
ggsave(
  filename = file.path(
    output_dir,
    "sensitivityAdmin1MeanWeight_simulation_direct_GVF_interval_score_95.png"
  ),
  plot = p_sz_095,
  width = 14,
  height = 6,
