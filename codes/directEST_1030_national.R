...
#' @param phantom_weight_quantile If NULL (default), the phantom cluster's weight is the national (per-stratum) mean cluster weight, as in the main paper. If set to a probability (e.g. 0.10), the phantom cluster's weight is instead the corresponding national (per-stratum) quantile of cluster weights -- for sensitivity analysis.
                          ...
                          ...)
{
  ...
  compute_domain_summaries_fix <- function(
    ...
    # ===== National-level cluster weight for phantom =====
    # Default: mean cluster weight (main-paper behaviour). If
    # phantom_weight_quantile is set (e.g. 0.10), the phantom cluster's
    # weight is instead the corresponding national quantile of total_weight,
    # per stratum -- used for the phantom-weight sensitivity analysis.
    if (is.null(phantom_weight_quantile)) {
      ...
      nat_avg_weight_h <- agg_cluster %>%
        group_by(strata) %>%
        summarise(
          nat_avg_total_weight = as.numeric(stats::quantile(
            total_weight, probs = phantom_weight_quantile, na.rm = TRUE
          )),
          .groups = "drop"
        )
    }
    ...
    if (var.fix == TRUE) {
      ...
      if (length(trigger_area) > 0) {
        ...
        for (i in trigger_area) {
          ...
          out <- compute_domain_summaries_fix(agg_cluster_in_out, n_h, target_region = admin2.full, p_h_hj_nat = p_h_hj_nat, admin.level = 2, phantom_weight_quantile = phantom_weight_quantile)
          df <-  out$df
    ...
    if (var.fix == TRUE) {
      ...
      if (length(trigger_area) > 0) {
        ...
        for (i in trigger_area) {
          ...
          out <- compute_domain_summaries_fix(agg_cluster_in_out, n_h, target_region = admin1, p_h_hj_nat = p_h_hj_nat, admin.level = 1, phantom_weight_quantile = phantom_weight_quantile)
          df <-  out$df
          df_pre_phantom <-  out$df_pre_phantom
